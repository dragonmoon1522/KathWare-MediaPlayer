<h2>KathWare Media Player</h2>

<input type="file" id="videoInput" accept="video/mp4,video/ogg,video/webm,audio/mp3">
<input type="file" id="subtitleInput" accept=".vtt,.srt">

<video id="videoPlayer" controls style="width: 100%; max-width: 900px; margin: 1rem 0;"></video>

<div class="controls" style="margin-bottom: 1rem;">
  <button onclick="video.play()">▶️ Reproducir</button>
  <button onclick="video.pause()">⏸️ Pausar</button>
  <button onclick="video.currentTime -= 10">⏪ -10s</button>
  <button onclick="video.currentTime += 10">⏩ +10s</button>
  <button onclick="video.volume = Math.min(video.volume + 0.1, 1)">🔊 +Vol</button>
  <button onclick="video.volume = Math.max(video.volume - 0.1, 0)">🔉 -Vol</button>
</div>

<label for="modoNarrador">Modo de lectura de subtítulos:</label>
<select id="modoNarrador" style="margin-bottom: 1rem;">
  <option value="off">Desactivado</option>
  <option value="sintetizador">Voz del sistema</option>
  <option value="lector">Usar lector de pantalla</option>
</select>

<label for="fuenteSub">Fuente de subtítulo:</label>
<select id="fuenteSub" style="margin-bottom: 1rem;">
  <option value="track">📄 Subtítulos (TRACK)</option>
  <option value="visual">📄 Closed Captions (CC)</option>
</select>

<div id="sub-accesible" aria-live="polite" role="alert" style="position: absolute; left: -9999px;"></div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function leerTextoAccesible(texto) {
    ...
    // Limpio tags HTML por las dudas
  ...
  // Arrancamos el modo visual si está seleccionado
